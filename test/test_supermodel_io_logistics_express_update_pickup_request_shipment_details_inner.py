# coding: utf-8

"""
    DHL Express APIs (MyDHL API)

    Welcome to the official DHL Express APIs (MyDHL API) below are the published API Documentation to fulfill your shipping needs with DHL Express.       Please follow the process described [here](https://developer.dhl.com/api-reference/dhl-express-mydhl-api#get-started-section/user-guide--get-access) to request access to the DHL Express - MyDHL API services   In case you already have DHL Express - MyDHL API Service credentials please ensure to use the endpoints/environments listed  [here](https://developer.dhl.com/api-reference/dhl-express-mydhl-api#get-started-section/user-guide--environments) 

    The version of the OpenAPI document: 2.11.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.supermodel_io_logistics_express_update_pickup_request_shipment_details_inner import SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner

class TestSupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner(unittest.TestCase):
    """SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner:
        """Test SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner`
        """
        model = SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner()
        if include_optional:
            return SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner(
                product_code = 'D',
                local_product_code = 'D',
                accounts = [
                    openapi_client.models.account_definition.Account definition(
                        type_code = 'shipper', 
                        number = '123456789', )
                    ],
                value_added_services = [
                    openapi_client.models.value_added_services_definition_for_/rates,_/products,_/landed_cost.Value Added Services definition for /rates, /products, /landed-cost(
                        service_code = 'II', 
                        local_service_code = 'II', 
                        value = 100, 
                        currency = 'GBP', 
                        method = 'cash', )
                    ],
                is_customs_declarable = True,
                declared_value = 150,
                declared_value_currency = 'CZK',
                unit_of_measurement = 'metric',
                shipment_tracking_number = '123456790',
                packages = [
                    openapi_client.models.package_definition_for_/rates,_/products,_/landed_cost.Package definition for /rates, /products, /landed-cost(
                        type_code = '3BX', 
                        weight = 10.5, 
                        dimensions = openapi_client.models.dimensions.dimensions(
                            length = 25, 
                            width = 35, 
                            height = 15, ), )
                    ]
            )
        else:
            return SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner(
                product_code = 'D',
                is_customs_declarable = True,
                unit_of_measurement = 'metric',
                packages = [
                    openapi_client.models.package_definition_for_/rates,_/products,_/landed_cost.Package definition for /rates, /products, /landed-cost(
                        type_code = '3BX', 
                        weight = 10.5, 
                        dimensions = openapi_client.models.dimensions.dimensions(
                            length = 25, 
                            width = 35, 
                            height = 15, ), )
                    ],
        )
        """

    def testSupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner(self):
        """Test SupermodelIoLogisticsExpressUpdatePickupRequestShipmentDetailsInner"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
